...
## alignment with BWA
bwa mem -aM -t $LSB_MAX_NUM_PROCESSORS $BWA_h37 ${R1[$LSB_JOBINDEX]} ${R2[$LSB_JOBINDEX]} | samtools view -bS - > $OUT/${MID}.bam  2> log/${MID}/${LSB_JOBID}_$(date "+%Y%m%d-%H%M%S").bwa.log
