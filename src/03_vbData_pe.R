...
cbd.df <- select_vbd_column(vbd, column = "lowess.ratio.quantal",
                            coord = chh)
write.table(cbd.df,
...
rbc.df <- select_vbd_column(vbd = vbd, column = "count",
                            coord = chh)
write.table(rbc.df,
            file = gzfile(file.path(outDir,
                                    paste(sample.name, "_", genome, ".", bin.size,
...
lbd <- select_vbd_column(vbd = vbd, column = "lowess.ratio",
                         coord = NULL)
## calculate mapd
...
read_varbin_pe_stats <- function(stats.file) {
    ...
    sst <- data.frame(t(ss[,2]))
    names(sst) <- paste(rownames(ss), "Reads", sep = ".")
    rownames(sst) = cell.id
    } else {
        sst <- data.frame(Total.Reads = 0, Passed.Reads = 0,
                          Filtered.Reads = 0, Counted.Reads = 0,
                          row.names = cell.id)
    }
    return(sst)
}
...
igv.seg$ID = gsub("varbin[0-9]+k..(.*)\\.(b[0-9]+)\\.(b[0-9]+).*", "\\1.\\2.\\3", igv.seg$ID)
igv.seg$chrom = gsub("[pq]", "", igv.seg$chrom)
## select(-start, -end, -num)
