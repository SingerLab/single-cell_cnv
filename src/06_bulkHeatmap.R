...
cnr20k[cnr20k > 4.5] <- 5
cnr20k[cnr20k <= 4.5 & cnr20k > 3.5] <- 4
cnr20k[cnr20k <= 3.5 & cnr20k > 2.5] <- 3
cnr20k[cnr20k <= 2.5 & cnr20k > 1.2] <- 2
cnr20k[cnr20k <= 1.2 & cnr20k > 0.2] <- 1
cnr20k[cnr20k < 0.2] <- 0

## Eliminate alterations in 5 or less cells
cnCount <- matrix(NA, nrow = 4999, ncol = 6, dimnames = list(colnames(1:4999), paste0("cn",c(0:5))))

sapply(1:4999, function(region) {
    cnCount[region, "cn0"] <<- sum(cnr[region, ] == 0)
    cnCount[region, "cn1"] <<- sum(cnr[region, ] == 1)
    cnCount[region, "cn2"] <<- sum(cnr[region, ] == 2)
    cnCount[region, "cn3"] <<- sum(cnr[region, ] == 3)    
    cnCount[region, "cn4"] <<- sum(cnr[region, ] == 4)
    cnCount[region, "cn5"] <<- sum(cnr[region, ] == 5)
})
cnCount <- data.frame(cnCount)
cnCount$keep <- apply(cnCount[, c(1:2,4:6)], 1, function(i) any(i > ceiling(ncol(cnr) * 0.05)))

## set all CN regions altered in <5% cells as 2
cnm <- cnr
## cnm[which(cnCount$keep == FALSE),] <- 2

bulkCNC <- data.frame(matrix(NA, nrow = length(Bulk), ncol = 6, dimnames = list(Bulk, paste0("cn",c(0:5)))))

sapply(Bulk, function(bulk) {
    bulkCNC[bulk, "cn0"] <<- sum(as.data.frame(cnr)[, bulk] == 0)
    bulkCNC[bulk, "cn1"] <<- sum(cnr[, bulk] == 1)
    bulkCNC[bulk, "cn2"] <<- sum(cnr[, bulk] == 2)
    bulkCNC[bulk, "cn3"] <<- sum(cnr[, bulk] == 3)    
    bulkCNC[bulk, "cn4"] <<- sum(cnr[, bulk] == 4)
    bulkCNC[bulk, "cn5"] <<- sum(cnr[, bulk] == 5)
})

normal.2n <- rownames(bulkCNC[bulkCNC$cn3 == 0 & bulkCNC$cn4 == 0 & bulkCNC$cn5 == 0 & bulkCNC$cn1 == 0 & bulkCNC$cn0 == 22,])
write.table(normal.2n, file = "varbin5k/2n-bulks.txt", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
cdd <- dist(t(cnm[,bulkCN$bulkID]))
pbdd <- vegdist(cdd, method = "bray")
...
## save.image("06_vbHeatmap.rda")

## deciding where to cut
## number of clusters by tree height
lapply(tree.height, function(he) {
    cct <- cutree(bcdd, h = he)
##     summaryp(cct)
    as.data.frame(table(cct))
    })

## cut tree screening 
png(file.path(figDir, paste(sample.name, "_MR_bulk_%03d.", aligner, bin.size, "_clusterOptimization.png", sep = "")), width = 410, height = 298, units = "mm", res = 400)
tree.height = seq(0.16, 0.30, by = 0.1)
sapply(tree.height, function(he) {
    cct <- cutree(bcdd, h = he)
    clColor <- rainbow(length(unique(cct)))
    names(clColor) <- paste("C", unique(cct), sep = "")
    myClusterSideBar <- clColor[cct]
    heatmap.2(cnm[, c(Bulk)], Rowv = FALSE, Colv = bcdd,
...
## cutting the tree
tree.height <- 0.19
cn5k.clust <- cutree(bcdd, h = tree.height)
...
nodePar  <- list(list(lab.cex = 0.6, pch = c(NA, 19), 
                cex = 0.7, col = bulkCN$clusterColor[bcdd$lables]))
dbcdd <- as.dendrogram(bcdd)
                
pdf(file.path(figDir, paste(sample.name, "_MR_bulk_dendrogram.pdf", sep = "")), width = 298/25.4, height = 149/25.4)
plot(bcdd, cex = 0.1, xlab = "", sub = "", lwd = 0.8)
plot(as.phylo(bcdd), type = "unrooted", cex = 0.4, tip.color = bulkCN[bcdd$labels, "clusterColor"], lwd = 1.3)
dev.off()
